...
pub const Registry = struct {
    ...
    pub fn insert(self: *Registry, gpa: std.mem.Allocator, owner: semantic.owner.Id, provider: Provider) Error!semantic.scene.FieldRef {
        ...
        try self.slots.append(gpa, .{ .owner = owner, .provider = provider });
        return .{ .authority = self.authority, .slot = @intCast(self.slots.items.len - 1), .generation = 1 };
    }

    pub fn get(self: *const Registry, ref: semantic.scene.FieldRef) ?Provider {
    ...
    pub fn remove(self: *Registry, gpa: std.mem.Allocator, owner: semantic.owner.Id, ref: semantic.scene.FieldRef) bool {
        ...
        if ((slot.owner orelse return false) != owner) return false;
    ...
    pub fn removeOwner(self: *Registry, gpa: std.mem.Allocator, owner: semantic.owner.Id) usize {
        var removed: usize = 0;
    ...
    fn retire(_: *Registry, _: std.mem.Allocator, slot: *Slot) void {
        slot.owner = null;
        slot.provider = null;
        slot.generation +%= 1;
...
test "field registry rejects a removed generation" {
    ...
    const first = try fields.insert(std.testing.allocator, owner, .init(&memory));
    try std.testing.expect(fields.get(first) != null);
    try std.testing.expect(!fields.remove(std.testing.allocator, other, first));
    try std.testing.expect(fields.remove(std.testing.allocator, owner, first));
    try std.testing.expect(fields.get(first) == null);
    const second = try fields.insert(std.testing.allocator, owner, .init(&memory));
    try std.testing.expectEqual(first.slot, second.slot);
    try std.testing.expect(first.generation != second.generation);
    try std.testing.expectEqual(@as(usize, 1), fields.removeOwner(std.testing.allocator, owner));
