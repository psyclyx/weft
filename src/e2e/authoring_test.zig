...
test "authoring/dired: rename a semantic field, apply its dialog, and verify disk" {
    ...
    const attached = (try fs.target.findEntry(file_descriptor.facts)) orelse return error.TestExpectedEqual;
    const authorized = try ed.session.system.filesystems.authorizedEntry(file_link.target, file_link.revision);
