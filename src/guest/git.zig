...
export fn on_fill() void {
    if (activeNameIs(buf_name)) {
        parseAndRender();
    } else if (activeNameIs("*git-diff*") or activeNameIs("*git-diff-staged*") or
        activeNameIs("*git-show*"))
    {
        classify(styleDiffLine);
    } else if (activeNameIs("*git-log*")) {
        classify(styleLogLine);
    } else if (activeNameIs("*git-rebase*")) {
