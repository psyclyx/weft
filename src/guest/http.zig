...
//! and streams the raw response into an *http* buffer. This is what turns the
...
/// GET arg0 (a URL); the raw HTTP response streams into *http*.
fn get() void {
    const raw = weft.argStr(0) orelse return;
    const u = parse(raw) orelse return;
    if (conn) |h| weft.netClose(h);
    weft.runStr("buffer-create", http_buf);
    conn = weft.netConnect(u.hostport, http_buf, if (u.tls) u.host else "");
