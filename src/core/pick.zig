...
// Matcher seam: the pure scoring/style functions live in `match.zig`;
// aliased here so the pick state + pool read them unqualified.
...
/// The pick's configuration/callback value types. See `pick/types.zig`.
...
/// The append-only, columnar candidate pool + its bulk `rank`. See
/// `pick/pool.zig`.
pub const Pool = @import("pick/pool.zig").Pool;
