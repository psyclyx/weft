...
/// What a guest said about one of its commands in `describe()`.
///
...
/// exactly like a core command, which has carried this metadata all along.
///
...
/// The entry whose projection this plugin has a build open on, captured at
/// `wl_proj_begin`. Generation-checked, so an entry closed mid-build resolves
/// to nothing rather than to whatever took its slot.
proj_building: ?Buffers.Ref = null,
...
pick_id: u32 = 0,
/// Whether the pick being accumulated accepts the TYPED query as well as a
/// listed row (`wl_pick_free_text`). Reset by every `wl_pick_begin`, so it is
...
pub fn declaresCommand(self: *WasmPlugin, name: []const u8) bool {
    return self.declaration(name) != null;
}
