...
//! regenerated (magit/dired) rather than a file. This is purely the buffer's
//! IDENTITY — it does NOT special-case dispatch. `save` (and any other context
//! intent) resolves through the ACTION system: the buffer's tool name is an
//! ambient fact (`action.Ctx.tool`, from `Editor.toolName`), and a projection
//! provides `save` scoped to `When{ .tool = "<name>" }`, winning over the core
//! file-write provider by specificity. So the core stays projection-agnostic.
