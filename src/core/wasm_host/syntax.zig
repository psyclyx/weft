...
/// `subbufferClear()`: release every subbuffer this plugin claimed (a projection
/// re-claims a fresh id-span per row each render — clear first so stale spans
/// from the previous render don't linger and confuse a reconcile). The handles
/// the guest held are invalidated; a projection must re-claim after.
pub fn hSubbufferClear(data: ?*anyopaque, caller: *wasm.Caller, args: []const i32, results: []i32) void {
    _ = caller;
    _ = args;
    _ = results;
    const p: *WasmPlugin = @ptrCast(@alignCast(data.?));
    const subs = p.subbuffers orelse return;
    for (p.subs.items) |sub| subs.release(p.gpa, sub);
    p.subs.clearRetainingCapacity();
}

/// `subbufferFactAt(offset, key) -> value`: read fact `key` off the INNERMOST
/// subbuffer covering `offset` (in the shared service — so it finds a span
/// claimed by ANY plugin, e.g. one the register re-stamped on paste), or -1 if
/// none / no such fact. The reconcile read-back: a projection reads each row's
/// hidden id to match edited rows to their original identity — path-independent,
/// so `dd`→`p` (the id ferried by the register) reads as a move.
pub fn hSubbufferFactAt(data: ?*anyopaque, caller: *wasm.Caller, args: []const i32, results: []i32) void {
    const p: *WasmPlugin = @ptrCast(@alignCast(data.?));
    const subs = p.subbuffers orelse {
        results[0] = -1;
        return;
    ...
    const key = caller.readMemory(p.gpa, @intCast(args[1]), @intCast(args[2])) catch {
        results[0] = -1;
        return;
    };
    defer p.gpa.free(key);
    const val = sub.fact(key) orelse {
        results[0] = -1;
        return;
    };
    results[0] = @intCast(caller.writeMemory(@intCast(args[3]), @intCast(args[4]), val) catch 0);
}
