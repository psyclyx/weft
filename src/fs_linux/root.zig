...
pub const LinuxFs = struct {
    ...
    const EntryState = struct {
        ...
        name: []u8,
        identity: Identity,
        /// Pins the exact inode so a delete/recreate cannot masquerade as the
        /// old opaque identity even if Linux later reuses its inode number.
        pinned_fd: i32,
    };
    ...
    pub fn list(
        ...
        while (true) {
            ...
            while (offset < bytes_read) {
                ...
                const entry = try self.mintEntry(root, directory, resolved.fd, name.bytes, snapshot.identity);
                const owned_name = try arena.dupe(u8, name.bytes);
                try entries.append(arena, .{
                    .name = contract.Name.init(owned_name) catch unreachable,
    ...
    fn createFile(self: *LinuxFs, root: contract.Root, create: anytype, outputs: []?contract.EntryRef) contract.Error!Execution {
        ...
        const snapshot = try statFd(fd);
        const entry = self.mintEntry(root, parent.node, parent.fd, create.destination.name.bytes, snapshot.identity) catch
    ...
    fn copy(self: *LinuxFs, destination_root: contract.Root, copy_op: anytype, outputs: []?contract.EntryRef) contract.Error!Execution {
        ...
        const entry = self.mintEntry(destination_root, destination.node, destination.fd, copy_op.destination.name.bytes, copied.identity) catch
    ...
    fn rename(self: *LinuxFs, destination_root: contract.Root, rename_op: anytype, outputs: []?contract.EntryRef) contract.Error!Execution {
        ...
        const entry = self.mintEntry(destination_root, destination.node, destination.fd, rename_op.destination.name.bytes, moved.identity) catch
    ...
    fn resolveEntry(self: *LinuxFs, root: contract.Root, entry: contract.EntryRef) contract.Error!ResolvedEntry {
        ...
        const pinned = try statFd(state.pinned_fd);
        if (!pinned.identity.eql(state.identity)) return error.Stale;
    ...
    fn observation(
        ...
        const link_target = if (snapshot.kind == .symlink) blk: {
            const target = try readlinkAt(self.gpa, parent_fd orelse return error.Io, name orelse return error.Io);
            defer self.gpa.free(target);
            ...
            break :blk try arena.dupe(u8, target);
        } else null;
        return .{
            .node = node,
    ...
    fn mintEntry(
        ...
        const name_z = try checkedNameZ(self.gpa, name);
        defer self.gpa.free(name_z);
        const pinned_fd = try fdResult(linux.openat(parent_fd, name_z.ptr, .{
            .PATH = true,
            .NOFOLLOW = true,
            .CLOEXEC = true,
        }, 0));
        errdefer closeFd(pinned_fd);
        const pinned = try statFd(pinned_fd);
        if (!pinned.identity.eql(identity)) return error.Stale;
        for (self.entries.items, 0..) |*slot, index| {
            ...
            if (state.identity.eql(identity)) {
                closeFd(pinned_fd);
                return .{
                    .authority = .here,
                    .slot = @intCast(index),
        ...
        for (self.entries.items, 0..) |*slot, index| {
            if (slot.value != null) continue;
            slot.value = .{ .root = root, .parent = parent, .name = owned_name, .identity = identity, .pinned_fd = pinned_fd };
            return .{ .authority = .here, .slot = @intCast(index), .generation = slot.generation };
        }
        try self.entries.append(self.gpa, .{
            .generation = self.generation_seed,
            .value = .{ .root = root, .parent = parent, .name = owned_name, .identity = identity, .pinned_fd = pinned_fd },
        });
        return .{
            .authority = .here,
...
fn revisionBytes(snapshot: *const LinuxFs.Snapshot) [revision_len]u8 {
    ...
    std.mem.writeInt(u32, bytes[28..32], snapshot.mode, .little);
    std.mem.writeInt(u64, bytes[32..40], snapshot.size, .little);
    std.mem.writeInt(u32, bytes[40..44], snapshot.nlink, .little);
    std.mem.writeInt(u64, bytes[44..52], @bitCast(snapshot.modified_sec), .little);
    std.mem.writeInt(u32, bytes[52..56], snapshot.modified_nsec, .little);
    std.mem.writeInt(u64, bytes[56..64], @bitCast(snapshot.changed_sec), .little);
    std.mem.writeInt(u32, bytes[64..68], snapshot.changed_nsec, .little);
    std.mem.writeInt(u32, bytes[68..72], @intFromEnum(snapshot.kind), .little);
    return bytes;
}
...
test "linux provider closes root fds and generation-checks reused slots" {
    var fixture = try Fixture.init(t.allocator);
    ...
    defer listing.deinit();
    const entry = findEntry(listing.value, "pinned") orelse return error.TestExpectedEqual;
    const entry_fd = (try fixture.local.entryState(entry.observation.node.entry)).pinned_fd;
    fixture.local.releaseRoot(first);
    try t.expectEqual(linux.E.BADF, linux.errno(linux.fcntl(first_fd, linux.F.GETFD, 0)));
    try t.expectEqual(linux.E.BADF, linux.errno(linux.fcntl(entry_fd, linux.F.GETFD, 0)));
