...
scion's wire ships v1 on stemma's existing per-unit-event encoding
...
Per the phase-2 rule (one encoding, owned by stemma — scion must not
...
Not implemented in scion; wire v1 negotiates versions precisely so the
